 ...
 *
 * @version     1.0
 * @author      Gravity Wiz <david@gravitywiz.com>
 * @license     GPL-2.0+
...
class GW_Tag_Editor {
    ...
    public function edit_form_tag( $tag, $form ) {
		...
		return sprintf( '<form %s>', $this->build_properties_string( $this->get_full_tag_properties( $tag ) ) );
    }
	 ...
	 */
	public function get_full_tag_properties( $tag ) {

		$custom_props  = $this->get_custom_properties();
		$default_props = $this->get_tag_properties( $tag );

		return wp_parse_args( $custom_props, $default_props );
	}
