<?php
/**
 * Gravity Forms Dynamics CRM Admin notices
 *
...
class GFDCRM_Admin_Notices {
	...
	/**
	 * Dismiss a Gravity Forms Dynamics CRM notice - stores the dismissed notices for 16 weeks
	 * @return void
	 */
	public function dismiss_notice() {
		...
		$dismissed_notices = (array)get_transient( 'gravityformsdynamicscrm_dismissed_notices' );
		...
		set_transient( 'gravityformsdynamicscrm_dismissed_notices', $dismissed_notices, WEEK_IN_SECONDS * 16 );
	...
	public function admin_notice() {
		 ...
		 * @since 1.3
		 */
		$notices = apply_filters( 'gravityformsdynamicscrm/admin/notices', self::$admin_notices );
		...
		//don't display a message if use has dismissed the message for this version
		self::$dismissed_notices = isset( $_GET['show-dismissed-notices'] ) ? array() : (array)get_transient( 'gravityformsdynamicscrm_dismissed_notices' );
		...
		foreach( $notices as $notice ) {
			...
			echo '<div id="message" class="notice '. gfdcrm_sanitize_html_class( $notice['class'] ).'">';
			...
			if( !empty( $notice['dismiss'] ) ) {
				...
				$url = esc_url( add_query_arg( array( 'gfdcrm-dismiss' => wp_create_nonce( 'dismiss' ), 'notice' => $dismiss ) ) );
	...
	public static function add_notice( $notice = array() ) {
		...
		if( !isset( $notice['message'] ) ) {
			do_action( 'gravityformsdynamicscrm_log_error', 'GFDCRM_Admin[add_notice] Notice not set', $notice );
			return;
		}
