<?php
/**
 * Functions that don't require Gravity Forms Dynamics CRM or Gravity Forms API access but are used in the plugin to extend PHP and WP functions
 * @since 1.3
 */
 ...
 */
function gfdcrm_is_not_empty_string( $mixed = '' ) {
	return ( $mixed !== '' );
}
 ...
 */
function gfdcrm_get_permalink_query_args( $id = 0 ) {
 ...
 */
function gfdcrm_sanitize_html_class( $classes ) {
 ...
 */
function gfdcrm_strip_whitespace( $string ) {
 ...
 */
function gfdcrm_ob_include( $file_path ) {
	if( ! file_exists( $file_path ) ) {
		do_action( 'gravityformsdynamicscrm_log_error', __FUNCTION__ . ': File path does not exist. ', $file_path );
		return '';
	}
	ob_start();
	include( $file_path );
 ...
 */
function gfdcrm_number_format( $number, $decimals = '' ) {
	global $wp_locale;
